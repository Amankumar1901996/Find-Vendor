...
public class VendorRegistation extends AppCompatActivity {
    ...
    public void customerRegisteration(View view) {
        Intent intent = new Intent(getApplicationContext(), CustomerRegister.class);
        startActivity(intent);
    }

    TextView vendorid, fullname, cityname, address, occupation, email, password;
    FirebaseAuth auth;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        password = (TextView) findViewById(R.id.cpassword);
        auth = FirebaseAuth.getInstance();
        ProgressBar progressBar = findViewById(R.id.progressBar);

        Button vregisterbtn = findViewById(R.id.vregisterBtn);
        vregisterbtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {

                String S_vendorid = vendorid.getText().toString().trim();
                ...
                String S_occupation = occupation.getText().toString().trim();
                String S_email = email.getText().toString().trim();
                String S_password = password.getText().toString();
                int iscustomer = 0;

                if (S_vendorid.isEmpty()) {
                    vendorid.setError("Please enter valid username");
                ...
                if (S_occupation.isEmpty()) {
                    occupation.setError("Please enter valid roll no. ");
                        ...
                        .addOnCompleteListener(new OnCompleteListener<AuthResult>() {
                            ...
                            public void onComplete(@NonNull Task<AuthResult> task) {
                                if (task.isSuccessful()) {
                                    ...
                                    VendorDataHolder userDataHolder = new VendorDataHolder(S_vendorid, S_fullname, S_cityname, S_address, S_occupation, S_email, S_password, iscustomer);

                                    FirebaseDatabase.getInstance().getReference("users")
                                            .child(FirebaseAuth.getInstance().getCurrentUser().getUid()).setValue(userDataHolder).addOnCompleteListener(new OnCompleteListener<Void>() {
                                        @Override
                                        public void onComplete(@NonNull Task<Void> task) {
                                            if (task.isSuccessful()) {
                                                progressBar.setVisibility(View.INVISIBLE);
                                                Log.i("realtime", "inside if of realtime");
                                                Toast.makeText(VendorRegistation.this, "successfully registered", Toast.LENGTH_SHORT).show();
                                                Intent activity2Intent = new Intent(getApplicationContext(), signIn.class);
                                                startActivity(activity2Intent);
                                            } else {
                                                Toast.makeText(VendorRegistation.this, "fail to register", Toast.LENGTH_SHORT).show();
                                            }
                                        }
                                    });
                                    ...
                                    Toast.makeText(VendorRegistation.this, "failed", Toast.LENGTH_SHORT).show();
                                }
                            }
                        });
